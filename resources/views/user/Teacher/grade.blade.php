...
<style>
    ...
    .sl-stats { display:flex; gap:0.75rem; flex-wrap: wrap; align-items: center; }
    ...
    .btn-save { background: #16a34a; color: #fff; border: none; padding: 0.6rem 1.2rem; border-radius: 6px; font-weight: 600; cursor: pointer; transition: background 0.2s; }
    .btn-save:hover { background: #15803d; }
    
    /* Input grade styling */
    .grade-input { width: 80px; padding: 0.4rem; border: 1px solid #cbd5e1; border-radius: 4px; text-align: center; font-weight: 600; outline: none; }
    .grade-input:focus { border-color: #16a34a; }
</style>

<div class="sl-wrapper">
    <div class="sl-header">
        <h1><i class="fa-solid fa-pen-to-square" style="color:#16a34a; margin-right:0.5rem;"></i>Cập nhật Điểm Sinh viên</h1>
    ...
    @if($currentSchedule)
        <div class="sl-card">
            <div class="sl-card-header">
                <h2>Lớp học phần: {{ $currentSchedule->subject->name ?? '' }} (Nhóm {{ $currentSchedule->group_code }})</h2>
                <div class="sl-stats">
                    <span style="font-size:0.85rem; font-weight: 600; color:#475569;"><i class="fa-solid fa-users"></i> Tổng số: {{ $students->count() }} SV</span>
                    <button class="btn-save" onclick="alert('Đã lưu điểm (Demo)')"><i class="fa-solid fa-save"></i> Chốt & Lưu điểm</button>
                </div>
            </div>
                ...
                <table class="sl-table" id="studentsTable">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center; width:150px;">Điểm tổng kết</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($students as $idx => $sv)
                            @php
                                ...
                                $initials = count($words) > 0 ? mb_substr($words[0], 0, 1) . (count($words) > 1 ? mb_substr(end($words), 0, 1) : '') : '';
                            @endphp
                            <tr>
                                <td style="font-weight:600; color:#94a3b8; text-align: center;">{{ $idx + 1 }}</td>
                                <td>
                                    <div class="sl-name-cell">
                                        <div class="sl-avatar" style="background:#f0fdf4; color:#16a34a;">{{ $initials ?: 'SV' }}</div>
                                ...
                                <td style="text-align:center;">
                                    <input type="number" class="grade-input" name="score[{{ $sv->enrollment_id }}]" min="0" max="10" step="0.1" value="{{ $sv->score }}" placeholder="--">
                                </td>
                            </tr>
                        @empty
                            <tr>
