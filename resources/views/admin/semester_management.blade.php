...
@section('content')
    <style>
        ...
        /* Active semester row highlight */
        tr.row-active{background:rgba(34,197,94,0.08);border-left:3px solid var(--success)}
        tr.row-active:hover{background:rgba(34,197,94,0.12)}
...
<div class="content-wrapper">
    <div class="breadcrumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a> / Quản lý Học kỳ</div>
    ...
    <div class="card">
        <div class="table-wrapper">
            <table>
                <thead><tr><th>ID</th><th>Tên học kỳ</th><th>Năm học</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
...
document.getElementById('semStatus').addEventListener('change', function() {
    document.getElementById('statusLabel').textContent = this.checked ? 'Đang hoạt động' : 'Không hoạt động';
});
...
function filterTable() {
    ...
    filteredData = allData.filter(s => (s.name||'').toLowerCase().includes(q) || (s.academic_year||'').toLowerCase().includes(q));
    currentPage = 1;
    renderPage();
}
...
function renderPage() {
    ...
    if (!filteredData.length) { tb.innerHTML = '<tr><td colspan="5"><div class="empty-state">Chưa có học kỳ nào</div></td></tr>'; }
    else {
